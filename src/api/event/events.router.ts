...
router.get("/list", isAuthenticated, validate(FilterEventDTO, "query"), canFilterByInstallation(), listEvents); // Check if the user is authenticated, validate filters, check if the filters are ok and call function listEvents
router.get("/export-list/:type", isAuthenticated, validate(FilterEventDTO, "query"), canFilterByInstallation(), exportEvents); //
